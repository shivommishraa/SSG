...
class CSR_Controller extends CI_Controller {
    ...
    $this->load->model(array('AgentCSR_model/CSR_model'));
    $this->load->model('Menu_model/Menu');
    $this->load->model(array('Admin_model/Role_model','Compaign_model/Compaign_model','Admin_model/User_model'));
  ...
  $config['base_url'] = base_url() ."CSR/CSR_Controller/manage_CSR"."/$fdate/$last_date";
  ...
  $this->load->view('CSR/agent_manage_csr',$data);
...
public function add_csr()
{
  ...
  $this->load->view('Dashboard/header.php',$data);
  $this->load->view('Dashboard/side.php');
  $this->load->view('CSR/add_csr',$data); 
  $this->load->view('Dashboard/footer.php');
}
   ...
   else
   {
     ...
     if(empty($adminid)){ 
       ...
       foreach($this->input->post('pro_val') as $fkey => $fvalue ){
         ...
         if(empty($check)){
          ...
          if(!empty($update))
          {
            ...
            $dataa['updated_by'] = $id; $dataa['pro_upd_val'] = $fvalue;
        ...
        if($checkbox!="upd")
        {
         ...
         if(!empty($gomanagepage)){
          ...
          redirect('CSR/CSR_Controller/manage_csr');}
        }
      }
      ...
      public function findprocess()
      {
        ...
        $this->load->view('CSR/Ajex_agent_form_Process', $data);
